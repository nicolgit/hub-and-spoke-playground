...
module fwPolicy './any-to-any-fw-policy.bicep' = {
  name: 'fwPolicyDeploy'
  params: {
...
resource subnetS01services 'Microsoft.Network/virtualNetworks/subnets@2020-05-01' = {
  ...
  dependsOn: [
    // possible race condition where the route table is being associated with two different subnets at the same time
    subnetS01default
  ]
  properties: {
