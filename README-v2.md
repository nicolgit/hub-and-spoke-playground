...
3. Set variables and create a resource group:
   ```bash
   LOCATION="westeurope"
...
4. Deploy the playground you need:
   ...
   # ANY-TO-ANY — Routing + firewall rules (requires HUB deployed first)
   az deployment group create --resource-group $RESOURCE_GROUP \
     --template-file any-to-any-bicep/any-to-any.bicep

   # S2S VPN — Site-to-site VPN (requires HUB + ON PREMISES deployed)
   az deployment group create --resource-group $RESOURCE_GROUP \
     --template-file s2s-vpn-bicep/connect-on-prem.bicep
...
> ⚠️ **Security Note:** Change these credentials immediately after deployment in production environments.
...
Deployed via [`hub-01-bicep/hub-01.json`](hub-01-bicep/hub-01.json):
...
Deployed via [`on-prem-bicep/on-prem.json`](on-prem-bicep/on-prem.json):

| Resource        | Description                                  |
| --------------- | -------------------------------------------- |
| `on-prem-net`   | VNet (France Central) simulating on-premises |
| `w10-onprem-vm` | Windows 10 client VM                         |
...
Deployed via [`on-prem-2-bicep/on-prem-2.json`](on-prem-2-bicep/on-prem-2.json):

| Resource        | Description                                               |
| --------------- | --------------------------------------------------------- |
| `on-prem-2-net` | VNet (Germany West Central) simulating second on-premises |
| `lin-onprem-vm` | Linux client VM                                           |
| Azure Bastion   | Secure access                                             |
| VPN Gateway     | S2S connectivity to hub                                   |
...
Deployed via [`hub-02-bicep/hub-02.json`](hub-02-bicep/hub-02.json):
...
Deployed via [`any-to-any-bicep/any-to-any.json`](any-to-any-bicep/any-to-any.json):
...
|   #   | Scenario                             | Solutions                                                                                                                                                                                                                                 |
| :---: | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|   1   | Any-to-any spoke communication       | [Azure Firewall](scenarios/ping-any-to-any-firewall.md) • [VNet Gateway](scenarios/ping-any-to-any-gateway.md) • [AVNM](scenarios/ping-any-to-any-avnm.md)                                                                                |
|   3   | Connect on-premises via VNet-to-VNet | [Solution](scenarios/vnet-to-vnet.md) • [Solution 2](scenarios/vnet-to-vnet-2.md)                                                                                                                                                         |
|   4   | Site-to-Site IPSec VPN               | [Basic](scenarios/ipsec.md) • [With BGP](scenarios/ipsec-bgp.md) • [Active-Active](scenarios/ipsec-active-active.md) • [Dual Redundancy](scenarios/ipsec-dual-redundancy.md) • [Multiple Devices](scenarios/ipsec-multiple-vpn-device.md) |
|   8   | Cross on-premises communication      | [Solution](scenarios/cross-on-premise-routing.md)                                                                                                                                                                                         |
|  14   | Routing with BGP                     | [VNet Gateway](scenarios/routing-with-bgp.md)                                                                                                                                                                                             |
|  15   | Routing without BGP                  | [Azure Firewall](scenarios/routing-without-bgp-fw.md)                                                                                                                                                                                     |
...
|   9   | North/South traffic inspection   | [Solution](scenarios/solution-north-south-inspection.md)             |
|  12   | Secure web workloads             | [Firewall + WAF](scenarios/publish-waf-fw.md)                        |
|  20   | Manage outbound internet traffic | [Azure Firewall](scenarios/outbound-traffic-to-internet-firewall.md) |
...
|  13   | Point-to-Site VPN              | [Certificate Auth](scenarios/p2s-vpn-certificate.md) • [Always-On](scenarios/p2s-vpn-certificate-always-on.md) |
|  16   | Publish internal web apps      | [Application Gateway](scenarios/app-gateway-01.md) • [Azure Front Door](scenarios/frontdoor.md)                |
|  17   | Publish SFTP endpoint          | [Azure Firewall](scenarios/sftp.md)                                                                            |
|  18   | Azure OpenAI with private APIM | [APIM + AOAI](scenarios/aoai.md)                                                                               |
|  19   | Azure Elastic SAN              | [Elastic SAN](scenarios/elastic-san.md)                                                                        |
